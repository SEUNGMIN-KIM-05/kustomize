pipeline {
  ...
  environment {
    ...
    awsecrRegistry = '851725651012.dkr.ecr.ap-northeast-2.amazonaws.com/eks-demo-repo'
  ...
  stages {
    ...
    stage('Checkout Application Git Branch') {
      steps {
        checkout([$class: 'GitSCM', branches: [[name: '*/main']], extensions: [], userRemoteConfigs: [[credentialsId: githubCredential, url: 'https://github.com/SEUNGMIN-KIM-05/kustomize.git']]])
      }
      // steps 가 끝날 경우 실행한다.
    ...
    stage('Deploy') { 
      steps {
        ...
        git credentialsId: githubCredential,
            url: 'https://github.com/SEUNGMIN-KIM/kustomize.git',
            branch: 'main'  
    ...
    stage('Push to Git Repository') {
      steps {
        withCredentials([[$class: 'UsernamePasswordMultiBinding', credentialsId: githubCredential, usernameVariable: 'SEUNGMIN-KIM-05', passwordVariable: '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']]) {
             sh "git push https://${SEUNGMIN-KIM-05}:${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}@github.com/SERUNGMIN-KIM/kustomize.git"       
        }
      }
    }
  }
